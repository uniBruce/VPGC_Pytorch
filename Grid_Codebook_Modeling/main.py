...
class ModelCheckpoint_MulSteps(ModelCheckpoint):
...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=True):
        ...
        self.log_steps = [2 ** n for n in range(int(np.log2(self.batch_freq)) + 1)][4:]
...
if __name__ == "__main__":
    ...
    if opt.name and opt.resume:
        print('has name and resume at the same time')
    if opt.resume:
        ...
        if os.path.isfile(opt.resume):
            ...
            ckptdir = opt.resume.rstrip("/")
            ckpt = os.path.join(ckptdir, "checkpoints", "last.ckpt")
