...
class ObamaDataset(Dataset):
    def __init__(self, data_config_path):
        ...
        if self.opt.isTrain:
            ...
            self.images = sorted(glob.glob(os.path.join(self.images_dir, '*.jpg')))[:] + sorted(glob.glob(os.path.join(self.images_dir, '*.png')))[:]
        else:
            self.images_dir = os.path.join(self.images_root, 'test')
            self.images = sorted(glob.glob(os.path.join(self.images_dir, '*.jpg')))[:1000] + sorted(glob.glob(os.path.join(self.images_dir, '*.png')))[:1000]
